...
const updateDoctorStatus = async (req, res) => {
    try {
        ...
        if (status === 'approved' && doctor.subscription.plan === 'trial') {
            ...
            doctor.subscription.subscriptionEndsAt = new Date(Date.now() + trialDays * 24 * 60 * 60 * 1000);
        }
...
const updateDoctorSubscription = async (req, res) => {
    try {
        ...
        if (!durationDays) {
            ...
            else if (plan === 'lifetime') durationDays = 36500; // 100 years
            else durationDays = 30;
        }
        doctor.subscription.subscriptionEndsAt = new Date(Date.now() + durationDays * 24 * 60 * 60 * 1000);
...
const getAnalytics = async (req, res) => {
    try {
        ...
        const approvedDoctors = await User.countDocuments({ role: 'doctor', accountStatus: 'approved' });
