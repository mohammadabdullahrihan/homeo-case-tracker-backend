...
const classifySymptomImportance = (symptom) => {
    ...
    const decisiveKeywords = ['sudden', 'violent', 'throbbing', 'intense', 'acute', 'rapid', 'burning', 'shooting', 'tearing', 'bursting', 'pulsating'];
    const characteristicModalities = ['worse from light', 'worse from noise', 'worse from jar', 'worse from motion', 'better from pressure', 'better from cold'];
...
const detectCasePattern = (symptoms) => {
    ...
    symptoms.forEach(sym => {
        ...
        if (text.includes('violent') || text.includes('intense') || text.includes('severe')) acuteSignals.violentIntensity = true;
...
const detectRemedyPattern = (remedyName, symptoms) => {
    ...
    return { matched, matchCount, matchStrength, pattern: pattern.description, isAcuteRemedy: pattern.acuteRemedy || false };
};

const calculatePolyChrestPenalty = (remedyName, stats) => {
    const bigRemedies = ['Belladonna', 'Sulphur', 'Calcarea Carbonica', 'Lycopodium', 'Phosphorus', 'Natrum Muriaticum', 'Arsenicum Album', 'Pulsatilla', 'Nux Vomica', 'Bryonia'];
...
const suggestRemedies = (symptoms, patientProfile = {}, limit = 10) => {
    try {
        ...
        const classifiedSymptoms = symptoms.map(sym => ({ ...sym, importance: classifySymptomImportance(sym) }));
        ...
        classifiedSymptoms.forEach((symObj, symIdx) => {
            ...
            repertoryData.categories.forEach(category => {
                ...
                category.rubrics.forEach(rubric => {
                    ...
                    const categoryMatch = searchTerms.some(term => categoryWords.some(word => word.includes(term) || term.includes(word)));
                    const matchedRubricCount = searchTerms.filter(term => rubricWords.some(word => word.includes(term) || term.includes(word))).length;

                    if ((categoryMatch && matchedRubricCount >= 1) || matchedRubricCount >= 2) {
                        const matchQuality = (matchedRubricCount + (categoryMatch ? 1 : 0)) / (searchTerms.length || 1);
                        rubric.remedies.forEach(rem => {
                            ...
                            if (!remedyStats[fullName]) {
                                remedyStats[fullName] = { rubricMatches: 0, totalGrade: 0, decisiveSymptomCount: 0, characteristicMatchCount: 0, abbr: rawAbbr, fullName: fullName, matchedSymptomTypes: new Set(), gradesArray: [] };
                            }
        ...
        const suggestions = Object.keys(remedyStats).map(name => {
            ...
            let acuteBonus = 0;
            if (patientProfile.type === 'acute' && casePattern.isAcuteCase && patternMatch.isAcuteRemedy) {
                acuteBonus = 20 + (casePattern.acuteSignalCount * 10);
            }

            const polyChrestPenalty = calculatePolyChrestPenalty(name, stats);
            const clinicalScore = ((baseScore + weightedIntensity) * multiplier * polyChrestPenalty) + patternBonus + acuteBonus;
...
const generateJustification = (remedyName, stats, patternMatch, acuteApplied) => {
    ...
    if (patternMatch.matched) {
        parts.push(`${remedyName}-এর বিশেষ ক্লিনিক্যাল প্যাটার্ন (${patternMatch.pattern}) এর সাথে ${patternMatch.matchStrength.toFixed(0)}% মিল পাওয়া গেছে`);
    }
